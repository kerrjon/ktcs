
using System.Web.Mvc;

namespace Ktcs.Areas.Admin.Controllers
{
  public class DemoController : Controller
  {
    public ActionResult Students() { return View(); }
    public ActionResult Surveys() { return View(); }
    public ActionResult Courses() { return View(); }
    public ActionResult Topics() { return View(); }
    public ActionResult Brands() { return View(); }
    public ActionResult Vendors() { return View(); }
    public ActionResult Payments() { return View(); }
    public ActionResult Waitlist() { return View(); }
    public ActionResult Classes() { return View(); }
    public ActionResult Locations() { return View(); }


    public ActionResult AdminBlog() { return View(); }
    public ActionResult AdminPricing() { return View(); }
    public ActionResult AdvancedDataTables() { return View(); }
    public ActionResult Animations() { return View(); }
    public ActionResult AutoMenu() { return View(); }
    public ActionResult BsCarousel() { return View(); }
    public ActionResult Buttons() { return View(); }
    public ActionResult Calendar() { return View(); }
    public ActionResult ChartBoxes() { return View(); }
    public ActionResult ChartJs() { return View(); }
    public ActionResult Chat() { return View(); }
    public ActionResult Checklist() { return View(); }
    public ActionResult CkEditor() { return View(); }
    public ActionResult Collapsable() { return View(); }
    public ActionResult ContentBoxes() { return View(); }
    public ActionResult DataTables() { return View(); }
    public ActionResult DropzoneUploader() { return View(); }
    public ActionResult FaqSection() { return View(); }
    public ActionResult FixedDatatables() { return View(); }
    public ActionResult FlotCharts() { return View(); }
    public ActionResult FormsElements() { return View(); }
    public ActionResult FormsMasks() { return View(); }
    public ActionResult FormsValidation() { return View(); }
    public ActionResult FormsWizard() { return View(); }
    public ActionResult Gmaps() { return View(); }
    public ActionResult HelperClasses() { return View(); }
    public ActionResult Icons() { return View(); }
    public ActionResult ImageCrop() { return View(); }
    public ActionResult Images() { return View(); }
    public ActionResult Index() { return View(); }
    public ActionResult IndexAlt() { return View(); }
    public ActionResult InlineEditor() { return View(); }
    public ActionResult InputKnobs() { return View(); }
    public ActionResult Invoice() { return View(); }
    public ActionResult JustGage() { return View(); }
    public ActionResult LabelsBadges() { return View(); }
    public ActionResult LazyLoad() { return View(); }
    public ActionResult LoadingFeedback() { return View(); }
    public ActionResult MailboxCompose() { return View(); }
    public ActionResult MailboxInbox() { return View(); }
    public ActionResult MailboxSingle() { return View(); }
    public ActionResult Mapael() { return View(); }
    public ActionResult Markdown() { return View(); }
    public ActionResult Modals() { return View(); }
    public ActionResult MorrisChat() { return View(); }
    public ActionResult MultiUploader() { return View(); }
    public ActionResult NavMenus() { return View(); }
    public ActionResult Notifications() { return View(); }
    public ActionResult PageTransistions() { return View(); }
    public ActionResult PanelBoxes() { return View(); }
    public ActionResult Pickers() { return View(); }
    public ActionResult PieGages() { return View(); }
    public ActionResult Popovers() { return View(); }
    public ActionResult PortfolioGallery() { return View(); }
    public ActionResult PortfolioMasonry() { return View(); }
    public ActionResult ProgressBars() { return View(); }
    public ActionResult ResponsiveTables() { return View(); }
    public ActionResult ResponsiveDataTables() { return View(); }
    public ActionResult ResponsiveMessages() { return View(); }
    public ActionResult Scrollbars() { return View(); }
    public ActionResult Sliders() { return View(); }
    public ActionResult SlideBars() { return View(); }
    public ActionResult SocialBoxes() { return View(); }
    public ActionResult SortableElements() { return View(); }
    public ActionResult Sparklines() { return View(); }
    public ActionResult SummerNote() { return View(); }
    public ActionResult Tables() { return View(); }
    public ActionResult Tabs() { return View(); }
    public ActionResult TileBoxes() { return View(); }
    public ActionResult Timeline() { return View(); }
    public ActionResult VectorMaps() { return View(); }
    public ActionResult ViewProfile() { return View(); }
    public ActionResult Xcharts() { return View(); }
  }
}